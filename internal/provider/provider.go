	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"space_id": schema.StringAttribute{
				Optional:    true,
				Description: "Contentful space ID. May also be set via the CONTENTFUL_SPACE_ID environment variable.",
			},
...
func (p *contentfulProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	spaceID := valueOrEnv(cfg.SpaceID, "CONTENTFUL_SPACE_ID")
	...
	requireAttr(resp, spaceID, "space_id", "CONTENTFUL_SPACE_ID")
	if resp.Diagnostics.HasError() {
		return
	}
